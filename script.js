...
const matches = [
  { id: "mex-can-20260611", home: "MEX", away: "CAN", kickoff: "2026-06-11T19:00:00-05:00", stadium: "Estadio Azteca" },
  { id: "usa-jpn-20260612", home: "USA", away: "JPN", kickoff: "2026-06-12T20:30:00-05:00", stadium: "MetLife Stadium" },
  { id: "bra-por-20260613", home: "BRA", away: "POR", kickoff: "2026-06-13T18:00:00-05:00", stadium: "SoFi Stadium" },
  { id: "arg-esp-20260614", home: "ARG", away: "ESP", kickoff: "2026-06-14T21:00:00-05:00", stadium: "AT&T Stadium" },
  { id: "fra-uru-20260615", home: "FRA", away: "URU", kickoff: "2026-06-15T17:30:00-05:00", stadium: "Lumen Field" },
  { id: "ger-eng-20260616", home: "GER", away: "ENG", kickoff: "2026-06-16T20:00:00-05:00", stadium: "Mercedes-Benz Stadium" },
  { id: "ned-mar-20260617", home: "NED", away: "MAR", kickoff: "2026-06-17T18:00:00-05:00", stadium: "Hard Rock Stadium" },
  { id: "ita-col-20260618", home: "ITA", away: "COL", kickoff: "2026-06-18T21:00:00-05:00", stadium: "Levi's Stadium" },
  { id: "mex-jpn-20260621", home: "MEX", away: "JPN", kickoff: "2026-06-21T19:00:00-05:00", stadium: "Estadio Akron" },
  { id: "bra-esp-20260622", home: "BRA", away: "ESP", kickoff: "2026-06-22T20:00:00-05:00", stadium: "Rose Bowl" },
  { id: "arg-eng-20260623", home: "ARG", away: "ENG", kickoff: "2026-06-23T20:30:00-05:00", stadium: "Gillette Stadium" },
  { id: "fra-por-20260624", home: "FRA", away: "POR", kickoff: "2026-06-24T18:30:00-05:00", stadium: "Lincoln Financial Field" }
];
...
function futureMatches(reference = new Date()) {
  return matches
    .filter((match) => matchDate(match).getTime() > reference.getTime())
...
function formatMatchDate(match) {
  ...
  }).format(matchDate(match)).replace(".", "");
}
...
function renderNextMatch() {
  const nextMatchCard = $("#nextMatchCard");
  const countdown = $("#matchCountdown");
  if (!nextMatchCard || !countdown) return null;
  ...
  if (!next) {
    nextMatchCard.innerHTML = `
      <div class="empty-state">
        ...
        <span>Actualiza la agenda en script.js cuando tengas nuevos encuentros.</span>
      </div>`;
    countdown.innerHTML = "";
    activeNextMatchId = "";
    return null;
  ...
  const home = teamByCode(next.home);
  const away = teamByCode(next.away);
  activeNextMatchId = next.id;

  nextMatchCard.innerHTML = `
    <div class="next-flags">
      <div class="next-team">
        ${flagMarkup(home)}
    ...
    <div class="match-meta">
      <span>${formatMatchDate(next)}</span>
      <span>${formatMatchTime(next)} h</span>
...
function renderCountdown(match) {
  const countdown = $("#matchCountdown");
  if (!countdown || !match) return;

  const diff = matchDate(match).getTime() - Date.now();
  const { days, hours, minutes, seconds } = timeParts(diff);

  countdown.innerHTML = `
    <span>Faltan</span>
    <div><strong>${pad(days)}</strong><small>Días</small></div>
    <div><strong>${pad(hours)}</strong><small>Horas</small></div>
    <div><strong>${pad(minutes)}</strong><small>Min</small></div>
    <div><strong>${pad(seconds)}</strong><small>Seg</small></div>`;
}

function refreshDynamicMatches() {
  const next = futureMatches()[0] || null;
  if (!next || next.id !== activeNextMatchId) {
    renderNextMatch();
    renderMatches();
  } else {
    renderCountdown(next);
...
function startCountdown() {
  ...
  renderNextMatch();
  renderMatches();
  countdownTimer = setInterval(refreshDynamicMatches, 1000);
}
...
function renderMatches() {
  ...
  const upcoming = futureMatches().slice(1, 5);
  if (!upcoming.length) {
    matchList.innerHTML = `<article class="match-row empty-row">No hay más partidos próximos en la agenda.</article>`;
    return;
  }

  matchList.innerHTML = upcoming.map((match) => {
    ...
    return `<article class="match-row">
      ...
      <div class="match-time">${formatMatchDate(match)}<br>${formatMatchTime(match)} h</div>
    </article>`;
  }).join("");
}
...
function initBankroll() {
  function calc() {
    const bank = Math.max(Number($("#bankrollAmount")?.value), 0);
    const odds = Math.max(Number($("#odds")?.value), 1);
    const probability = clamp(Number($("#probability")?.value) || 0, 1, 99) / 100;
    const kellyFraction = Number($("#risk")?.value) || 0.25;
    const maxStakePct = clamp(Number($("#maxStake")?.value) || 3, 0.25, 10);
    const decimalProfit = odds - 1;
    const edge = probability * odds - 1;
    const kellyBase = decimalProfit > 0 ? ((decimalProfit * probability) - (1 - probability)) / decimalProfit : 0;
    const rawStakePct = Math.max(0, kellyBase * kellyFraction * 100);
    const stakePct = edge > 0 ? clamp(rawStakePct, 0, maxStakePct) : 0;
    const stake = bank * (stakePct / 100);
    ...
    if (status) {
      ...
      const statusClass = edge <= 0 ? "negative" : stakePct >= maxStakePct ? "capped" : "positive";
      const statusText = edge <= 0
        ? "No apostar: la cuota no supera la probabilidad estimada."
        : stakePct >= maxStakePct
          ? "Valor positivo, pero el tope por pick está protegiendo el bankroll."
          : "Valor positivo: stake calculado con Kelly fraccionado.";

      status.className = `bank-status ${statusClass}`;
      status.innerHTML = `
        <strong>${statusText}</strong>
        <span>Cuota justa ${fairOdds.toFixed(2)} · Prob. implícita ${pct(impliedProbability * 100)} · Prob. IA ${pct(probability * 100)}</span>`;
    }

    $("#stakeResult").textContent = money(stake);
    $("#stakePctResult").textContent = `${stakePct.toFixed(2)}%`;
    $("#edgeResult").textContent = `${(edge * 100).toFixed(1)}%`;
    $("#kellyResult").textContent = `${Math.max(kellyBase * 100, 0).toFixed(1)}%`;
    $("#returnResult").textContent = money(ret);
    $("#profitResult").textContent = money(profit);
  }

  function applyAiPick() {
    const linkedPick = $("#linkedPick");
    if (!latestPrediction?.recommendation) {
      if (linkedPick) linkedPick.textContent = "Primero analiza un partido en el predictor IA.";
      return;
    }

    const probabilityInput = $("#probability");
    if (probabilityInput) probabilityInput.value = latestPrediction.recommendation.probability.toFixed(1);
    if (linkedPick) {
      linkedPick.textContent = `Cargado: ${latestPrediction.recommendation.label} · prob. IA ${pct(latestPrediction.recommendation.probability)}. Ajusta la cuota real antes de apostar.`;
    }
    calc();
  }

  ["bankrollAmount", "odds", "probability", "risk", "maxStake"].forEach((id) => {
    $("#" + id)?.addEventListener("input", calc);
    $("#" + id)?.addEventListener("change", calc);
  });
  $("#applyAiPick")?.addEventListener("click", applyAiPick);
  calc();
}
